...
function addFabricConstructionForBbblOrTt()
{
    ...
    if (!fabric_construction.val()) {
        ...
        let tr = `<tr>
                    ...
                    <td><input type="hidden" class="select-fabric-construction-id" name="fabric_construction_ids[]" value="${ fabric_construction.val() }">${ fabric_construction.text() }</td>
...
{
    let style_id = $(object).closest('tr').find('.select-fabric-construction-id').val()
    $("#select-fabric-construction option[value=" + style_id + "]").removeAttr('disabled')
    $("#select-fabric-construction").val('').trigger('chosen:updated');
...
function setFcprice(evnt)
{
    ...
    if(keycode == '13') {
        evnt.preventDefault();
        addFabricConstructionForBbblOrTt()
    }
