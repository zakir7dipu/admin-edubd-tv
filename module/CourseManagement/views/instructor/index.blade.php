...
@section('content')
    <div class="breadcrumbs ace-save-state" id="breadcrumbs">
        ...
        <ul class="breadcrumb mb-1">
            ...
            <li><a class="text-muted" href="{{ route('cm.instructors.index') }}">Instructors</a></li>
        </ul>
    </div>
    <form method="GET">
        <div class="row mt-2">
            ...
            <div class="col-md-4 text-right">
                <div class="btn-group">
                    ...
                    <a class="btn btn-sm btn-theme" data-toggle="tooltip" data-placement="top" title="Add New" href="{{ route('cm.instructors.create') }}" class="btn btn-sm btn-black" style="height: 33px; line-height: 22px; border-top-left-radius: 2px; border-bottom-left-radius: 2px">
    ...
    <div class="row mt-2">
        <div class="col-md-12">
                            ...
                            <td>
                                <div style="display: flex; align-items: center; gap: 5px">
                                    @if ($instructor->image != '' && file_exists($instructor->image))
                                        <img src="{{ asset($instructor->image) }}" class="img-thumbnail img-circle" width="50px" height="50px" alt="{{ $instructor->username }}">
                                    @else
                                        <img src="{{ asset('assets/img/default.png') }}" class="img-thumbnail img-circle" width="50px" height="50px" alt="{{ $instructor->username }}">
                                    @endif
                                    <div style="display: flex; flex-direction: column; gap: 2px; line-height: 10px">
                                        <p style="color: black; font-weight: 500">{{ $instructor->first_name }} {{ $instructor->last_name }}</p>
                                        <b style="color: blue">{{ '@' . $instructor->username }}</b>
                                    </div>
                                </div>
                            </td>
                            <td>{{ $instructor->bio }}</td>
                            <td>
                                <a style="text-decoration: none" href="mailto:{{ $instructor->email }}">{{ $instructor->email }}</a>
                            </td>
                            <td>
                                <a style="text-decoration: none" href="tel:{{ $instructor->phone }}">{{ $instructor->phone }}</a>
                            </td>
                            <td>{{ $instructor->gender }}</td>
                            <td class="text-center">
                                <x-status status="{{ $instructor->status }}" id="{{ $instructor->id }}" table="{{ $table }}" />
                            </td>
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if (hasPermission('instructor-edit', $slugs))
                                        <li>
                                            <a href="{{ route('cm.instructors.edit', $instructor->id) }}" title="Edit">
                                                <i class="far fa-edit"></i> Edit
                                            </a>
                                        </li>
                                        @endif
                                        @if (hasPermission('instructor-delete', $slugs))
                                        <li>
                                            <a href="javascript:void(0)" title="Delete" onclick="delete_item('{{ route('cm.instructors.destroy', $instructor->id) }}')" type="button">
            ...
            @include('partials._paginate', ['data' => $instructors])
        </div>
    </div>
@endsection
