...
class CourseCategoryUpdateRequest extends FormRequest
{
    ...
    {
        return [
            'name'      => ['required', 'min:2', 'max:255', 'unique_with:cm_course_categories,name,parent_id,' . $this->course_category->id],
            'slug'      => ['required', 'min:2', 'max:255', 'regex:/^[a-zA-Z0-9-]+$/', 'unique:cm_course_categories,slug,' . $this->course_category->id],
            'serial_no' => ['required', 'numeric', 'unique:cm_course_categories,serial_no,' . $this->course_category->id],
        ];
    }
