...
class CourseCategoryStoreRequest extends FormRequest
{
    ...
    {
        return [
            'name'      => ['required', 'min:2', 'max:255', 'unique_with:cm_course_categories,name,parent_id'],
            'slug'      => ['required', 'min:2', 'max:255', 'regex:/^[a-zA-Z0-9-]+$/', 'unique:cm_course_categories,slug'],
            'serial_no' => ['required', 'numeric', 'unique:cm_course_categories,serial_no'],
        ];
    }
