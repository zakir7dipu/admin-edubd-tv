...
class ExamCategoryStoreRequest extends FormRequest
{
    ...
    {
        return [
            'name'      => ['required', 'min:2', 'max:255', 'unique_with:em_exam_categories,name,parent_id'],
            'slug'      => ['required', 'min:2', 'max:255', 'regex:/^[a-zA-Z0-9-]+$/', 'unique:em_exam_categories,slug'],
            'serial_no' => ['required', 'numeric', 'unique:em_exam_categories,serial_no'],
        ];
    }
