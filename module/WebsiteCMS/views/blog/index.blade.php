        ...
        <div class="row mt-2">
            <div class="col-md-4">
                <div class="input-group width-100" style="width: 100%">
                    ...
                    <select name="category_id" id="category_id" class="select2 form-control">
                        ...
                        @foreach ($blogCategories as $id => $name)
            ...
            <div class="col-md-4 text-right">
                <div class="btn-group">
                    ...
                    <a class="btn btn-sm btn-theme" data-toggle="tooltip" data-placement="top" title="Add New" href="{{ route('wc.blog.create') }}" class="btn btn-sm btn-black" style="height: 33px; line-height: 22px; border-top-left-radius: 2px; border-bottom-left-radius: 2px">
            ...
            <table id="myTable" class="table table-bordered">
                <thead style="border-bottom: 3px solid #346cb0 !important">
                    <tr style="background: #e1ecff !important; color:black !important">
                        ...
                        <th class="text-center" width="10%">Thambnail Image</th>
                        <th class="text-center" width="10%">Cover Image</th>
                        <th class="text-center" width="10%">Short Description</th>
                ...
                <tbody>
                    @forelse ($blogs as $blog)
                        <tr>
                            ...
                            <td>
                                <img src="{{ asset($blog->thumbnail_image) }}" width="100%" height="100px" alt="">
                            </td>
                            <td>
                                <img src="{{ asset($blog->cover_image) }}" width="100%" height="100px" alt="">
                            </td>
                            <td class="text-center">{{ $blog->short_description }}</td>
                            <td class="text-center">{{ $blog->serial_no }}</td>

                            <td class="text-center">
                                <x-status status="{{ $blog->status }}" id="{{ $blog->id }}" table="{{ $table }}" />
                            </td>
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if(hasPermission("blog-edit", $slugs))
                                        <li>
                                            <a href="{{ route('wc.blog.edit', $blog->id) }}" title="Edit">
                                                <i class="far fa-edit"></i> Edit
                                            </a>
                                        </li>
                                        @endif
                                        @if(hasPermission("blog-delete", $slugs))
                                        <li>
                                            <a href="javascript:void(0)" title="Delete" onclick="delete_item('{{ route('wc.blog.destroy', $blog->id) }}')" type="button">
