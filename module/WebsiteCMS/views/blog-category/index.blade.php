...
@section('content')
        ...
        <div class="row mt-2">
            ...
            <div class="col-md-4 text-right">
                <div class="btn-group">
                    ...
                    @if(hasPermission("blog-cetegory-create", $slugs))
                    <a class="btn btn-sm btn-theme" data-toggle="tooltip" data-placement="top" title="Add New" href="{{ route('wc.blog-category.create') }}" class="btn btn-sm btn-black" style="height: 33px; line-height: 22px; border-top-left-radius: 2px; border-bottom-left-radius: 2px">
    ...
    <div class="row mt-2">
        <div class="col-md-12">
            ...
            <table id="myTable" class="table table-bordered">
                <thead style="border-bottom: 3px solid #346cb0 !important">
                    <tr style="background: #e1ecff !important; color:black !important">
                        ...
                        <th class="text-center" width="5%">Blog</th>
                ...
                <tbody>
                    @forelse ($blogCategories as $blogCategory)
                        <tr>
                            ...
                            <td class="">
                                <a href="{{ $blogCategory->slug }}" target="_blank">
                                    {{ $blogCategory->name }}
                                </a>
                            </td>
                            <td>{{ $blogCategory->slug }}</td>
                            <td class="text-center">
                                <a href="{{ route('wc.blog.index') }}?category_id={{ $blogCategory->id }}" class="badge" style="background: #5000db; font-weight: 600">
                                    {{ $blogCategory->totalBlog }}
                                </a>
                            </td>
                            <td class="text-center">{{ $blogCategory->serial_no }}</td>

                            <td class="text-center">
                                <x-status status="{{ $blogCategory->status }}" id="{{ $blogCategory->id }}" table="{{ $table }}" />
                            </td>
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if(hasPermission("blog-cetegory-edit", $slugs))
                                        <li>
                                            <a href="{{ route('wc.blog-category.edit', $blogCategory->id) }}" title="Edit">
                                                <i class="far fa-edit"></i> Edit
                                            </a>
                                        </li>
                                        @endif
                                        @if(hasPermission("blog-cetegory-delete", $slugs))
                                        <li>
                                            <a href="javascript:void(0)" title="Delete" onclick="delete_item('{{ route('wc.blog-category.destroy', $blogCategory->id) }}')" type="button">
            ...
            @include('partials._paginate', ['data' => $blogCategories])
        </div>
    </div>
@endsection
